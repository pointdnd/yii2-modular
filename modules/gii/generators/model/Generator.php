...
class Generator extends \mii\modules\gii\Generator
{
    ...
    public function beforeRequest($controller,$id)
    {
        if(isset($_GET['gid']) and isset($_GET['delete'])) {
            $modelGenerator = ModelGenerator::findOne($_GET['gid']);
            $modelGenerator->delete();
            $controller->redirect(['/gii/default/view','id'=>$id]);
        }
    }

    public function afterSuccessSave($controller,$id)
    {
       if(!isset($_GET['gid'])) {
            
            $modelGenerator = ModelGenerator::find()
        ...
        $modelGenerator->attributes=$this->attributes;
        $modelGenerator->templates=\yii\helpers\Json::encode($this->templates);
        $modelGenerator->created_at=time();
        if(!$modelGenerator->save())
        {
            // var_dump($modelGenerator->errors);
            // exit;
        }
    }
    ...
    {
        $model = ModelGenerator::find()->orderBy('created_at DESC')->all();
        foreach($model as $data) {
            ...
            echo '<a href="'.$url.'">...'.substr($data->modelClass, -20).'</a> <small class="text-muted">'.$data->tableName.'</small>';
            echo '<br>';
            echo '<em>'.$data->ns.'</em>';
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            if(!file_exists(Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '.php')) {
                $files[] = new CodeFile(
