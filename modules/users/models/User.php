...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    {
        foreach($this->module->admins as $role) {
    ...
    public function attributeLabels()
    {
        return [
            ...
            'unconfirmed_email' => \Yii::t('users', 'New email'),
            'password'          => \Yii::t('users', 'Password'),
            'created_at'        => \Yii::t('users', 'Registration time'),
            'confirmed_at'      => \Yii::t('users', 'Confirmation time'),
        ];
    }
    ...
    public function attemptConfirmation($code)
    {
        ...
        if ($token === null || $token->isExpired) {
            \Yii::$app->session->setFlash('danger', \Yii::t('users', 'Confirmation link is invalid or out-of-date. You can try requesting a new one.'));
        }
        ...
        if ($this->save(false)) {
            \Yii::$app->session->setFlash('success', \Yii::t('users', 'Your account has been successfully confirmed.'));
        } else {
            \Yii::$app->session->setFlash('danger', \Yii::t('users', 'Something went wrong and your account has not been confirmed.'));
        }
    }
    ...
    protected function getFlashMessage()
    {
        if ($this->module->enableGeneratingPassword && $this->module->enableConfirmation) {
            return \Yii::t('users', 'A message has been sent to your email address. It contains your password and a confirmation link that you must click to complete registration.');
        } else if ($this->module->enableGeneratingPassword) {
            return \Yii::t('users', 'A message has been sent to your email address. It contains a password that we generated for you.');
        } else if ($this->module->enableConfirmation) {
            return \Yii::t('users', 'A message has been sent to your email address. It contains a confirmation link that you must click to complete registration.');
        } else {
            return \Yii::t('users', 'Welcome! You have been successfully registered and logged in.');
        }
    }
    ...
    {
        return static::find(1)->one();
        if($type=='yii\\filters\\auth\\HttpBasicAuth')
            return static::find()->where(['email'=>$token])->one();
        // $token here is access_token
        if($type=='yii\\filters\\auth\\HttpBearerAuth')
            return static::find()->where(['access_token'=>$token])->one();
        if($type=='yii\\filters\\auth\\QueryParamAuth')
            return static::find()->where(['access_token'=>$token])->one();
        throw new NotSupportedException('"Access Token" is not valid.');
    }
}
